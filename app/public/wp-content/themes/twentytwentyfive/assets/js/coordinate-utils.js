...
(function (global) {
    ...
    class GridManager {
         ...
         *
         * isPointInFill only works on visible elements. The click pad paths have
         * display:none set by CSS, so we temporarily override that for the check,
         * then restore it.  If the element is unavailable we trust the transform.
         */
        geoToSVG(lat, lon, regionName) {
            ...
            // Temporarily reveal the path so isPointInFill works
            ...
            // Re-reveal for the walk (each step needs a fresh hit-test)
            el.style.setProperty('display', 'block');
            const STEPS = 40;
